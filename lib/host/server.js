 ...
 *
 * Chinese, matching the plugin's `webLang` default: the report labels, the time
 * series and the drill-down all read more naturally in it. `?lang=en` still
 * switches a single request.
 */
 ...
 * @param {'en' | 'zh'} [options.lang] default label language for the page.
...
export async function startDashboardServer(usageStats, options = {}) {
        ...
        try {
          ...
          state.html = view === 'text'
            ...
            : renderDashboard(report, {
              // No title override here: `renderDashboard` derives a title from the
              // language, and hardcoding one is how an English <title> survived on
              // an otherwise Chinese page.
  ...
  async function handle(req, res) {
    try {
      ...
      // The configured default applies unless the URL names a language, so a
      // deployment can ship a Chinese page while `?lang=en` still works.
      const requested = url.searchParams.get('lang')
      const lang = requested === 'zh' || requested === 'en'
        ? requested
...
function textDocument(report, lang, title) {
  const heading = title ?? (lang === 'zh' ? 'DSH Token 用量报告' : 'DSH token usage')
...
<title>${escapeHtml(heading)} — text</title>
