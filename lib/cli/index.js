...
Options:
  ...
  --max-sessions <n>      Keep only the N heaviest sessions in the report
  --max-calls <n>         Keep only the N heaviest calls in the report
...
export async function run(argv, io = {}) {
  ...
  const retain = command === 'calls' || command === 'sessions' || command === 'turns'
    || command === 'dashboard' || command === 'export' || showDetail === 'full'
    ? 'full'
  ...
  switch (command) {
    ...
    case 'dashboard': {
      // The dashboard embeds one row per call, so a caller may cap how many
      // sessions it carries; the report itself is unaffected.
